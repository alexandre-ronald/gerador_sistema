...
class GeneratedProjectQualityValidator:
    ...
    @staticmethod
    def _is_allowed_empty_python(path: Path) -> bool:
        """Return True for legitimate empty Python package markers."""
    ...
    def validate(self) -> tuple[str, ...]:
        ...
        for artifact in artifacts:
            ...
            is_allowed_empty = (
                ...
                and path.suffix == ".py"
                and self._is_allowed_empty_python(path)
            )

            if artifact.kind != "static" and is_empty and not is_allowed_empty:
        ...
        for py_path in self.root.rglob("*.py"):
            ...
            for pattern in self._TEMPLATE_PATTERNS:
                for template_name in pattern.findall(content):
                    candidates = [self.root / "templates" / template_name]
                    ...
                    if len(parts) > 1:
                        ...
                        candidates.append(
                            self.root / app_name / "templates" / relative_template
                        )
