...
class RegistroUsuarioForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists(): raise forms.ValidationError("Este e-mail já está cadastrado no sistema.")
        return email
    def clean(self):
        cleaned = super().clean()
        if cleaned.get("password") and cleaned.get("confirm_password") and cleaned["password"] != cleaned["confirm_password"]: self.add_error("confirm_password", "As senhas não coincidem.")
        return cleaned
...
def registrar_usuario_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user); messages.success(request, f"Seja bem-vindo(a), {user.first_name}!"); return redirect("sistema:dashboard")
    else: form = RegistroUsuarioForm()
    return render(request, "registration/registro.html", {"form": form})

@login_required
def lista_sistemas(request): return render(request, "sistema/lista.html", {"sistemas": Sistema.objects.filter(usuario=request.user).order_by("-atualizado_em")})

@login_required
def criar_sistema(request):
    default_path = str(Path(getattr(settings, "BASE_DIR", Path.cwd())) / "projetos_gerados")
    return render(request, "sistema/editor_moderno.html", {"estrutura_json": json.dumps({"sistema": {"caminho": default_path}, "modulos": []}, ensure_ascii=False), "sistema_id": None})

@login_required
def editar_sistema(request, sistema_id):
    sistema = get_object_or_404(Sistema, pk=sistema_id, usuario=request.user)
    return render(request, "sistema/editor_moderno.html", {"estrutura_json": json.dumps(serialize_system_structure(sistema), ensure_ascii=False), "sistema_id": sistema.id})

@login_required
@require_http_methods(["POST"])
def salvar_modelo(request):
    try:
        payload = json.loads(request.body or "{}"); sistema = save_system_structure(user=request.user, payload=payload, sistema_id=payload.get("sistema_id"))
        return JsonResponse({"status": "sucesso", "sistema_id": sistema.id, "caminho_geracao": sistema.caminho_geracao})
    except Exception as exc: return JsonResponse({"status": "erro", "mensagem": str(exc)}, status=400)

@login_required
@require_http_methods(["PUT", "POST"])
def atualizar_sistema(request, sistema_id):
    try:
        payload = json.loads(request.body or "{}"); sistema = save_system_structure(user=request.user, payload=payload, sistema_id=sistema_id)
        return JsonResponse({"status": "sucesso", "sistema_id": sistema.id, "caminho_geracao": sistema.caminho_geracao})
    except Exception as exc: return JsonResponse({"status": "erro", "mensagem": str(exc)}, status=400)
...
def excluir_sistema(request, sistema_id):
    sistema = get_object_or_404(Sistema, pk=sistema_id, usuario=request.user); nome = sistema.nome; sistema.delete(); messages.success(request, f"Sistema '{nome}' excluído com sucesso!"); return redirect("sistema:lista")

@login_required
def gerar_sistema_view(request, pk):
    sistema = get_object_or_404(Sistema, pk=pk, usuario=request.user); total_entidades = Entidade.objects.filter(modulo__sistema=sistema).count()
...
def gerar_sucesso_view(request, pk):
    sistema = get_object_or_404(Sistema, pk=pk, usuario=request.user); return render(request, "sistema/gerar_sucesso.html", {"sistema": sistema, "nome_projeto": GeradorService._python_identifier(sistema.nome, "projeto"), "caminho": sistema.caminho_geracao})
...
def baixar_zip_sistema(request, pk):
    ...
    if not sistema.arquivo_zip or not os.path.exists(sistema.arquivo_zip.path): raise Http404("O arquivo ZIP deste sistema não foi encontrado.")
