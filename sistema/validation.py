...
def validate_specification(sistema: Sistema) -> None:
    ...
    if sistema.banco_dados not in SUPPORTED_DATABASES:
        errors.append(
            f"Banco de dados '{sistema.banco_dados}' não é suportado nesta versão. Use SQLite ou PostgreSQL."
        )
    ...
    for modulo in modulos:
        ...
        for entidade in modulo.entidades.all():
            ...
            if key in entity_names:
                errors.append(f"Entidades duplicadas no módulo '{modulo.nome}': '{entidade.nome}'.")
            ...
            for campo in entidade.campos.all():
                ...
                if field_name in field_names:
                    errors.append(f"Campos duplicados em '{entidade.nome}': '{campo.nome}'.")
                field_names.add(field_name)

                if campo.tipo not in SUPPORTED_FIELD_TYPES:
                    errors.append(f"Tipo de campo não suportado: '{campo.tipo}' em '{entidade.nome}.{campo.nome}'.")

                if campo.tipo == "DecimalField":
                    if campo.max_digits is None or campo.max_digits <= 0:
                        errors.append(f"DecimalField '{entidade.nome}.{campo.nome}' precisa de max_digits.")
                    if campo.decimal_places is None or campo.decimal_places < 0:
                        errors.append(f"DecimalField '{entidade.nome}.{campo.nome}' precisa de decimal_places.")
                    if campo.max_digits is not None and campo.decimal_places is not None and campo.decimal_places > campo.max_digits:
                        errors.append(f"DecimalField '{entidade.nome}.{campo.nome}' possui decimal_places maior que max_digits.")

                if campo.tipo in {"CharField", "EmailField", "URLField"} and (not campo.max_length or campo.max_length <= 0):
                    errors.append(f"{campo.tipo} '{entidade.nome}.{campo.nome}' precisa de max_length.")
    ...
    if sistema.usar_custom_user:
        errors.append("Custom User ainda não faz parte do GEN-0001. Desative 'Gerar Custom User Model' antes da geração.")

    if sistema.gerar_api_rest or any(entidade.gerar_endpoints_api for modulo in modulos for entidade in modulo.entidades.all()):
        errors.append("A geração de API REST será habilitada em uma fase posterior. Desative API REST para gerar com o GEN-0001.")
