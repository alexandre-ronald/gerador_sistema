...
class WorkspacePreviewTests(SimpleTestCase):
    ...
    def test_destination_urls_are_projected_from_stable_refs(self):
        preview=project_workspace_preview(self.structure,role_simulation=self.role("gestor"),entities=self.entities); items={item["id"]:item for item in preview["selected"]["sections"][0]["items"]}; self.assertEqual(items["fornecedores"]["url"],"?entidade=10&pagina=list&workspace=gestao&workspace_item=fornecedores"); self.assertEqual(items["relatorio"]["url"],"?entidade=20&pagina=report&relatorio=ativos&workspace=gestao&workspace_item=relatorio")
    def test_destination_url_preserves_workspace_context_for_reload(self):
        preview=project_workspace_preview(self.structure,role_simulation=self.role("gestor"),entities=self.entities,selected_workspace_id="gestao"); item=preview["selected"]["sections"][0]["items"][1]; self.assertIn("workspace=gestao",item["url"]); self.assertIn("workspace_item=contratos",item["url"])
