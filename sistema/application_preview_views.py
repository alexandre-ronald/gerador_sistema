...
def _advanced_page_from_designer_referer(request, sistema):
    ...
    first_enabled = next(
        (page for page in config.get("pages", []) if page.get("enabled")),
        None,
    )
...
def _advanced_record_entity_id(sistema, selected_page_id):
    ...
    page = next(
        (
            ...
            if item.get("id") == selected_page_id and item.get("enabled")
        ),
        None,
    )
    ...
    return (
        Entidade.objects.filter(
            modulo__sistema=sistema,
            nome=context.get("entity"),
        )
    ...
    if page_kind == "advanced" and preview.get("advanced_page"):
        contextual.append(
            link(
                ...
                "page_designer",
                "bi-window-stack",
                query=f"pagina={preview['advanced_page']['id']}",
            )
        )
    elif page_kind == "form":
        contextual.append(
...
def _apply_report_permissions(sistema, preview):
    ...
    def allowed(report):
        if invalid_role:
            return False
        entity_policies = (
            report_policies.get(report.get("entity"))
            if isinstance(report_policies, dict)
            else None
        )
    ...
    visible_reports = [
        report for report in (preview.get("reports") or []) if allowed(report)
    ]
    ...
    report_page = (
        original_report_page
        if isinstance(original_report_page, dict) and allowed(original_report_page)
        else None
    )
    preview["report_page"] = report_page
    preview.setdefault("navigation", {})["reports"] = _report_navigation_rows(
        visible_reports,
        report_page,
    )
    ...
    if preview.get("page_kind") == "report" and original_report_page and report_page is None:
        ...
        preview["content"] = {
            ...
            "subtitle": (
                "O Permission Designer não autorizou o papel simulado a acessar este relatório."
            ),
        }
    ...
    for workspace in workspace_preview.get("navigation_groups") or []:
        ...
        for section in workspace.get("sections") or []:
            ...
            if items:
                ...
                sections.append(section_projection)
                navigation["workspace_sections"].append(
                    {
                        **section_projection,
                        "id": f"{workspace.get('id')}:{section.get('id')}",
                        "label": (
                            f"{workspace.get('label') or 'Workspace'} · "
                            f"{section.get('label') or 'Seção'}"
                        ),
                    }
                )
...
def application_preview(request, sistema_id):
    ...
    referer_advanced_page = (
        _advanced_page_from_designer_referer(request, sistema)
        if not explicit_page_kind
        else ""
    )
    requested_page_kind = explicit_page_kind or (
        "advanced" if referer_advanced_page else "list"
    )
    selected_advanced_page = (
        request.GET.get("pagina_avancada") or referer_advanced_page
    )
    selected_entity_id = request.GET.get("entidade")

    if not selected_entity_id and requested_page_kind == "advanced":
        selected_entity_id = _advanced_record_entity_id(
            sistema,
            selected_advanced_page,
        )
    ...
    structure = _draft_structure(sistema)
    entities = list(
        Entidade.objects.filter(modulo__sistema=sistema).order_by("nome", "id")
    )
    preview["workspace_preview"] = project_workspace_preview(
        structure,
