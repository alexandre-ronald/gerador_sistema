...
PREVIEW_DEVICES={"desktop":{"id":"desktop","label":"Desktop","icon":"bi-display","width":1280},"tablet":{"id":"tablet","label":"Tablet","icon":"bi-tablet","width":820},"mobile":{"id":"mobile","label":"Mobile","icon":"bi-phone","width":390}}
def _draft_structure(sistema):
    versao=sistema.versoes.filter(numero=0).first(); return versao.estrutura_json if versao and isinstance(versao.estrutura_json,dict) else {}
def _device_preview(raw_device):
    requested=str(raw_device or "desktop").strip().lower(); selected=requested if requested in PREVIEW_DEVICES else "desktop"; return {"selected":selected,"requested":requested,"invalid":requested not in PREVIEW_DEVICES,"options":[dict(item) for item in PREVIEW_DEVICES.values()],**PREVIEW_DEVICES[selected]}
def _advanced_page_from_designer_referer(request,sistema):
    ...
    config=normalize_advanced_pages_config(_draft_structure(sistema).get("advanced_pages"),strict=False); first_enabled=next((page for page in config.get("pages",[]) if page.get("enabled")),None); return str((first_enabled or {}).get("id") or "").strip()
def _advanced_record_entity_id(sistema,selected_page_id):
    ...
    config=normalize_advanced_pages_config(_draft_structure(sistema).get("advanced_pages"),strict=False); page=next((item for item in config.get("pages",[]) if item.get("id")==selected_page_id and item.get("enabled")),None); context=(page or {}).get("context") or {}
...
def _ensure_workflow_navigation(sistema,preview):
    navigation=preview.setdefault("navigation",{}); projected=navigation.get("workflows"); workflows=list(projected) if isinstance(projected,list) else []; known_entities={str(item.get("entity") or "") for item in workflows if isinstance(item,dict)}; stored=_draft_structure(sistema).get("workflows"); stored=stored if isinstance(stored,dict) else {}
    if not stored:navigation["workflows"]=workflows;return
    entities={entity.nome:entity for entity in Entidade.objects.filter(modulo__sistema=sistema).select_related("modulo").order_by("nome","id")}; selected_entity_id=(preview.get("workflow_page") or {}).get("entity_id") if isinstance(preview.get("workflow_page"),dict) else None
    for entity_name,config in stored.items():
        ...
        if entity is not None:workflows.append({"entity_id":entity.pk,"entity":entity.nome,"label":entity.nome_plural or entity.nome,"icon":"bi-diagram-3","active":selected_entity_id==entity.pk})
...
def _apply_report_permissions(sistema,preview):
    ...
    raw_rbac=_draft_structure(sistema).get("rbac"); raw_rbac=raw_rbac if isinstance(raw_rbac,dict) else {}; report_policies=raw_rbac.get("reports")
