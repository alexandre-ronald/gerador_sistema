...
def page_designer(request, sistema_id):
    ...
    return render(request, "sistema/page_designer_metrics.html", {"sistema": sistema, "advanced_pages_json": json.dumps(config, ensure_ascii=False), "entities_json": json.dumps(metadata, ensure_ascii=False), "designer_catalog_json": json.dumps(_designer_catalog(estrutura), ensure_ascii=False)})
