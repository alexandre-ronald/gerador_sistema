...
def build_spec(audit=False):
    entity = EntitySpec("Pessoa", "Pessoa", "pessoa", "Pessoas", "", True, True, False, ())
    module = ModuleSpec("Cadastro", "cadastro", "", (entity,))
    return SystemSpec("2.3", "Sistema Teste", "sistema_teste", "sistema-teste", "", "sqlite3", "lateral", False, False, False, audit, (module,))
...
class Gen0013ListagemTests(SimpleTestCase):
    ...
    def test_generated_list_view_has_pagination_and_search(self):
        compiled = SpecificationCompiler(build_spec()).compile()
        item = next(x for x in compiled if x.path == "cadastro/views.py")
...
class Gen0014AuditTests(SimpleTestCase):
    ...
    def test_audit_enabled_generates_complete_audit_app(self):
        ...
        for path in (
            ...
            "auditoria/migrations/0001_initial.py",
        ):
    ...
    def test_audit_artifacts_are_written(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue((Path(tmp) / "auditoria" / "migrations" / "0001_initial.py").exists())
