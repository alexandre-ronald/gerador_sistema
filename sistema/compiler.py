...
class SpecificationCompiler:
    ...
    def compile(self):
        ...
        for artifact in self.plan.artifacts():
            # Somente artefatos explicitamente estáticos podem ser vazios.
            # Artefatos de auditoria, módulos e templates sempre precisam ser
            # materializados com conteúdo.
            ...
            if not content.strip():
                raise RuntimeError(
                    f"Template compilado vazio: {artifact.path} ({template_name})"
                )
    ...
    def write(self, output_directory):
        ...
        expected = {i.path for i in self.plan.artifacts()}
        ...
        if {i.path for i in compiled} != expected:
    ...
    def _template_for(self, artifact):
        mapping = {
            ...
            "urls.py:module": "gerador/snippets/urls_app_gen0012.txt",
            "admin.py": "gerador/snippets/admin.txt",
            "apps.py": "gerador/snippets/apps_config.txt",
            "_list.html": "gerador/snippets/html_list_gen0013.txt",
            "_detail.html": "gerador/snippets/html_detail.txt",
            ...
            "auditoria_apps.txt": "gerador/snippets/auditoria_apps.txt",
            "auditoria_models.txt": "gerador/snippets/auditoria_models.txt",
            "auditoria_admin.txt": "gerador/snippets/auditoria_admin.txt",
            "auditoria_middleware.txt": "gerador/snippets/auditoria_middleware.txt",
            "auditoria_request_context.txt": "gerador/snippets/auditoria_request_context.txt",
            "auditoria_signals.txt": "gerador/snippets/auditoria_signals.txt",
            "0001_initial.py": "gerador/snippets/auditoria_migration.txt",
        }
        path, name = artifact.path, Path(artifact.path).name
        if artifact.kind == "audit":
            if path.endswith("/admin.py"):
                return mapping["auditoria_admin.txt"]
            if path.endswith("/apps.py"):
                return mapping["auditoria_apps.txt"]
            if path.endswith("/models.py"):
                return mapping["auditoria_models.txt"]
            if path.endswith("/middleware.py"):
                return mapping["auditoria_middleware.txt"]
            if path.endswith("/request_context.py"):
                return mapping["auditoria_request_context.txt"]
            if path.endswith("/signals.py"):
                return mapping["auditoria_signals.txt"]
            if path.endswith("migrations/0001_initial.py"):
                return mapping["0001_initial.py"]
            if path.endswith("migrations/__init__.py"):
                return mapping["__init__.py"]
            return mapping["__init__.py"]
        ...
        if artifact.kind == "crud":
            ...
            if name.endswith("_detail.html"):
                return mapping["_detail.html"]
    ...
    def _module_adapter(self, module):
        return SimpleNamespace(
            ...
            entidades=_Collection(self._entity_adapter(e) for e in module.entities),
        )
                ...
                for candidate in cm.entities:
                    if candidate.class_name == field.related_entity:
                        related_entity = SimpleNamespace(
                            ...
                            modulo=SimpleNamespace(nome=cm.name, nome_tecnico=cm.technical_name),
                        )
                        break
                if related_entity:
                    break
...
class ArtifactWriter:
    ...
    def write(self, artifacts):
        ...
        for artifact in artifacts:
            ...
            if self.output_directory not in destination.parents:
                raise ValueError(f"Artefato fora do diretório permitido: {artifact.path}")
            destination.parent.mkdir(parents=True, exist_ok=True)
...
def compile_specification(specification):
    return SpecificationCompiler(specification).compile()


def write_compiled_specification(specification, output_directory):
    return ArtifactWriter(output_directory).write(SpecificationCompiler(specification).compile())
