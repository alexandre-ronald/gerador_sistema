...
def _experience_catalog(structure,entities):
    ...
    for page in advanced.get("pages",[]):
        if page.get("enabled") and (page.get("context") or {}).get("kind") != "record": experiences.append({"kind":"advanced_page","ref":page["id"],"label":page.get("name") or page["id"],"group":"Páginas de negócio"})
    raw_cruds=structure.get("cruds") if isinstance(structure.get("cruds"),dict) else {}
    for entity in entities:
        if not raw_cruds or entity.nome in raw_cruds: experiences.append({"kind":"crud","ref":entity.nome,"operation":"list","label":entity.nome,"group":"Cadastros e consultas"})
    dashboard=normalize_dashboard_config(structure.get("dashboard"))
    if dashboard.get("enabled"): experiences.append({"kind":"dashboard","ref":"dashboard","label":dashboard.get("title") or "Dashboard","group":"Painéis"})
    ...
    for entity_name,collection in reports.items():
        ...
        for report in items:
            if isinstance(report,dict) and report.get("enabled") and report.get("id"): experiences.append({"kind":"report","ref":f"{entity_name}:{report['id']}","label":report.get("title") or report.get("name") or report["id"],"group":"Relatórios"})
    entity_names={entity.nome for entity in entities}; workflows=structure.get("workflows") if isinstance(structure.get("workflows"),dict) else {}
    for entity_name,workflow in workflows.items():
        if entity_name in entity_names and isinstance(workflow,dict) and workflow.get("enabled") is True: experiences.append({"kind":"workflow","ref":entity_name,"label":f"Fluxo de {entity_name}","group":"Fluxos de trabalho"})
    return experiences

@login_required
def workspace_designer(request,sistema_id):
    sistema=get_object_or_404(Sistema,pk=sistema_id,usuario=request.user); structure=_draft_structure(sistema); entities=_entities(sistema); raw_config=structure.get("workspaces") if isinstance(structure.get("workspaces"),dict) else None; config=normalize_workspace_config(raw_config,strict=False)
    return render(request,"sistema/workspace_designer.html",{"sistema":sistema,"workspaces_json":json.dumps(config,ensure_ascii=False),"experience_catalog_json":json.dumps(_experience_catalog(structure,entities),ensure_ascii=False)})
...
def salvar_workspace_designer(request,sistema_id):
    ...
    except (WorkspaceContractError,WorkspaceSemanticError) as exc: return JsonResponse({"status":"erro","erro":exc.as_dict(),"mensagem":exc.message},status=400)
    except (TypeError,ValueError,json.JSONDecodeError) as exc: return JsonResponse({"status":"erro","mensagem":f"Configuração inválida: {exc}"},status=400)
