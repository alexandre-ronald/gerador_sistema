...
def _empty_config():
    return {"enabled": False, "id": "", "title": "", "description": "", "navigation": {"path": [], "label": ""}, "columns": [], "filters": [], "order_by_code": ""}
...
def _navigation(entidade, raw, title):
    raw_nav = raw.get("navigation") if isinstance(raw.get("navigation"), dict) else {}
    path = raw_nav.get("path") if isinstance(raw_nav.get("path"), list) else []
    path = [str(item).strip() for item in path if str(item).strip()]
    if not path:
        path = [str(getattr(entidade, "nome", "Relatórios"))]
    return {"path": path, "label": str(raw_nav.get("label") or title).strip() or title}
...
def _config_from_raw(entidade, raw, index=1):
    ...
    return {"enabled": True,"id": report_id,"title": title,"description": str(raw.get("description") or ""),"navigation": _navigation(entidade, raw, title),"columns": columns,"filters": filters,"order_by_code": order_code,"view_name": f"{entity_code}_report_{report_id}","template_name": f"{entity_code}_report_{report_id}.html"}
...
def report_generation_config(entidade, report_id=None):
    ...
    if report_id:
        for config in configs:
            if config.get("id") == report_id: return config
        return _empty_config()
