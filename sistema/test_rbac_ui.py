...
class PermissionDesignerUITests(TestCase):
    ...
    def test_workflow_actions_keep_transition_contract(self):
        content=self.client.get(self.url).content.decode()
        self.assertIn("toggleTransition",content);self.assertIn("t.id",content);self.assertIn("t.label",content)
        self.assertIn("Permitir que ${esc(r.label)} execute esta ação no processo.",content)
    ...
    def test_save_persists_role_description_and_preserves_other_draft_keys(self):
        ...
        draft=VersaoGeracao.objects.get(sistema=self.sistema,numero=0);self.assertIn("forms",draft.estrutura_json);self.assertIn("workflows",draft.estrutura_json);self.assertTrue(draft.estrutura_json["rbac"]["enabled"])
        roles={item["id"]:item for item in draft.estrutura_json["rbac"]["roles"]};self.assertEqual(roles["gestor"]["description"],"Aprova e acompanha pedidos.");self.assertEqual(roles["gestor"]["group"],"Gestores")
        self.assertEqual(draft.estrutura_json["rbac"]["entities"]["Pedido"]["roles"]["operador"],["list","view","create"]);self.assertEqual(draft.estrutura_json["rbac"]["entities"]["Pedido"]["transitions"]["aprovar"],["gestor"])
