...
class WorkspaceVisibilityTests(SimpleTestCase):
    ...
    def test_projection_orders_workspaces_sections_and_items_by_order_stably(self):
        item=lambda item_id,label,order:{"id":item_id,"label":label,"order":order,"destination":{"kind":"crud","ref":"Contrato","operation":"list"}}
        config={"version":1,"default_workspace":"segundo","workspaces":[{"id":"segundo","label":"Segundo","order":20,"home":"item_b","sections":[{"id":"secao_b","label":"Seção B","order":20,"items":[item("item_b","Item B",20),item("item_a","Item A",10)]},{"id":"secao_a","label":"Seção A","order":10,"items":[item("item_c","Item C",0)]}]},{"id":"primeiro","label":"Primeiro","order":10,"home":"item_d","sections":[{"id":"principal","label":"Principal","order":0,"items":[item("item_d","Item D",0)]}]},{"id":"empate","label":"Empate","order":10,"home":"item_e","sections":[{"id":"principal","label":"Principal","order":0,"items":[item("item_e","Item E",0)]}]}]}
        projected=visible_workspace_config(config,structure=self.structure,rbac=self.rbac,role_ids=["gestor"])
        self.assertEqual([workspace["id"] for workspace in projected["workspaces"]],["primeiro","empate","segundo"])
        segundo=projected["workspaces"][2]; self.assertEqual([section["id"] for section in segundo["sections"]],["secao_a","secao_b"]); self.assertEqual([item["id"] for item in segundo["sections"][1]["items"]],["item_a","item_b"])
