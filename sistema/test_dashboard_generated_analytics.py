...
class GeneratedDashboardAnalyticsTests(SimpleTestCase):
    ...
    def test_runtime_supports_safe_filters(self):
        source = self.compact_source()
        self.assertIn("ifoperator=='neq':qs=qs.exclude", source)
        self.assertIn("lookup=FILTER_LOOKUPS.get(operator)", source)
        self.assertIn("ifnot_field(model,name):raiseValueError('Filtroinválido')", source)
    ...
    def test_runtime_keeps_widget_local_error_isolation(self):
        source = self.compact_source()
        self.assertIn("exceptException:item['error']='Nãofoipossívelexecutaraconsultadestewidget.'", source)
