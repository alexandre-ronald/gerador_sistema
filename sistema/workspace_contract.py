...
def _normalize_item(raw, *, workspace_id, section_id):
    if not isinstance(raw, dict):
        raise WorkspaceContractError("invalid_workspace_item", "Item deve ser um objeto.", workspace_id=workspace_id, section_id=section_id)
    item_id = _safe_id(raw.get("id"), code="invalid_workspace_item_id", workspace_id=workspace_id, section_id=section_id)
    return {
        "id": item_id,
        "label": _label(raw.get("label"), code="empty_workspace_item_label", workspace_id=workspace_id, section_id=section_id, item_id=item_id),
        "icon": str(raw.get("icon") or "").strip(),
        "enabled": _bool(raw.get("enabled", True), code="invalid_workspace_item_enabled", workspace_id=workspace_id, section_id=section_id, item_id=item_id),
        "order": _int(raw.get("order", 0), code="invalid_workspace_item_order", workspace_id=workspace_id, section_id=section_id, item_id=item_id),
        "destination": _normalize_destination(raw.get("destination"), workspace_id=workspace_id, section_id=section_id, item_id=item_id),
    }


def _normalize_section(raw, *, workspace_id):
    if not isinstance(raw, dict):
        raise WorkspaceContractError("invalid_workspace_section", "Seção deve ser um objeto.", workspace_id=workspace_id)
    section_id = _safe_id(raw.get("id"), code="invalid_workspace_section_id", workspace_id=workspace_id)
    raw_items = raw.get("items", [])
    if not isinstance(raw_items, list):
        raise WorkspaceContractError("invalid_workspace_items", "Itens devem ser uma lista.", workspace_id=workspace_id, section_id=section_id)
    items = []
    item_ids = set()
    for raw_item in raw_items:
        item = _normalize_item(raw_item, workspace_id=workspace_id, section_id=section_id)
        if item["id"] in item_ids:
            raise WorkspaceContractError("duplicate_workspace_item_id", "ID de item duplicado no workspace.", workspace_id=workspace_id, section_id=section_id, item_id=item["id"])
        item_ids.add(item["id"])
        items.append(item)
    return {
        "id": section_id,
        "label": _label(raw.get("label"), code="empty_workspace_section_label", workspace_id=workspace_id, section_id=section_id),
        "icon": str(raw.get("icon") or "").strip(),
        "enabled": _bool(raw.get("enabled", True), code="invalid_workspace_section_enabled", workspace_id=workspace_id, section_id=section_id),
        "order": _int(raw.get("order", 0), code="invalid_workspace_section_order", workspace_id=workspace_id, section_id=section_id),
        "items": items,
    }
...
def _normalize_workspace(raw):
    if not isinstance(raw, dict):
        raise WorkspaceContractError("invalid_workspace", "Workspace deve ser um objeto.")
    workspace_id = _safe_id(raw.get("id"), code="invalid_workspace_id")
    raw_sections = raw.get("sections", [])
    if not isinstance(raw_sections, list):
        raise WorkspaceContractError("invalid_workspace_sections", "Seções devem ser uma lista.", workspace_id=workspace_id)
    sections = []
    section_ids = set()
    item_ids = set()
    for raw_section in raw_sections:
        ...
        if section["id"] in section_ids:
            raise WorkspaceContractError("duplicate_workspace_section_id", "ID de seção duplicado.", workspace_id=workspace_id, section_id=section["id"])
        duplicate_item = next((item["id"] for item in section["items"] if item["id"] in item_ids), None)
        if duplicate_item:
            raise WorkspaceContractError("duplicate_workspace_item_id", "ID de item deve ser único no workspace.", workspace_id=workspace_id, section_id=section["id"], item_id=duplicate_item)
        section_ids.add(section["id"])
        item_ids.update(item["id"] for item in section["items"])
        sections.append(section)
    home = str(raw.get("home") or "").strip()
    if home and home not in item_ids:
        raise WorkspaceContractError("unknown_workspace_home", "Home referencia item inexistente no workspace.", workspace_id=workspace_id, item_id=home)
    return {
        "id": workspace_id,
        "label": _label(raw.get("label"), code="empty_workspace_label", workspace_id=workspace_id),
        "description": str(raw.get("description") or "").strip(),
        "icon": str(raw.get("icon") or "").strip(),
        "enabled": _bool(raw.get("enabled", True), code="invalid_workspace_enabled", workspace_id=workspace_id),
        "order": _int(raw.get("order", 0), code="invalid_workspace_order", workspace_id=workspace_id),
        "home": home,
        "sections": sections,
    }
...
def normalize_workspace_config(raw_config, *, strict=True):
    ...
    if not isinstance(raw_workspaces, list):
        if strict:
            raise WorkspaceContractError("invalid_workspaces", "Workspaces devem ser uma lista.")
        raw_workspaces = []
    workspaces = []
    workspace_ids = set()
    for raw_workspace in raw_workspaces:
        ...
        if workspace["id"] in workspace_ids:
            if strict:
                raise WorkspaceContractError("duplicate_workspace_id", "ID de workspace duplicado.", workspace_id=workspace["id"])
            continue
    ...
    if default_workspace and default_workspace not in workspace_ids:
        if strict:
            raise WorkspaceContractError("unknown_default_workspace", "Workspace default referencia workspace inexistente.", workspace_id=default_workspace)
        default_workspace = ""
...
def workspace_map(config):
    ...
    return {workspace["id"]: deepcopy(workspace) for workspace in normalized["workspaces"]}


def workspace_item_map(config, workspace_id):
    workspace = workspace_map(config).get(workspace_id)
    if workspace is None:
        raise WorkspaceContractError("unknown_workspace", "Workspace inexistente.", workspace_id=workspace_id)
